...
| `/scorecard` direct traffic, weekly | | | GA4 property `G-YRK73W9NS9` → Pages and screens, path `/scorecard`, Direct channel. Reporting identity must be **Device-based**, Google Signals off (§2 reading rule 5) |
...
| **Visits** | Unique visits for the week | GA4 property `G-YRK73W9NS9`, Device-based identity, Signals off | GA4 at this volume is a traffic-mix thermometer, not an attribution system (§2 reading rule 5). Thresholding blanks rows under ~40–50 users while Signals is on; the free BigQuery export bypasses thresholds entirely |
| **Signups** | Accounts created during the week | `GET /api/admin/growth-funnel?days=7` → `signups`; the real-time founder Telegram signup pings are the running cross-check | The DB is ground truth. UTM-attributed signups are a **floor** — in-app browsers and cross-device undercount, and every Facebook click arrives via the `l.facebook.com` shim, so `signup_referrer_host` cannot separate paid from organic |
| **Card-adds** | Gated-cohort accounts that completed the `/app/start` card wall | SQL over `users`: `created_at >= '2026-08-22'` AND `trial_started_at IS NOT NULL`, excluding `is_admin`, `is_lifetime` and hand-comped paid rows — mirroring the exemptions in `tier.must_add_card` | **No event fires on gate completion** (gap G1), so the SQL is the only read today. The `/app/start` redirect also fires for grandfathered accounts and for bookmarked subscribers who were never gated, which is why the cohort is defined by `created_at` rather than by who landed on the page |
...
| **Payers** | Subscriptions past the trial with a charge collected | `GET /api/admin/revenue` → `active_subscriptions` / `subs_by_status["active"]`; Stripe → Payments as the tiebreak | **Do not use `paid_customers`, `acquisition_channels[*].paid`, or `growth-funnel`'s `paying`.** All three count `stripe_customer_id IS NOT NULL`, which the card gate now stamps at TRIAL start — so from 2026-08-22 they conflate trials with payers. That is gap G12, unfixed, and it inflates every per-channel paid tally |
