...
The inbox auto-handler ships across PRs #173 / #187 / #190.
...
The Reddit poller is gated on the account having enough karma to
post in r/stocks. See `docs/growth/reddit_karma_comments.md` for the
karma-building plan. Don't wire Reddit credentials until karma is
built — the poller would silently fail on every comment-reply
attempt.
...
2. ```powershell
   fly secrets set `
     ...
     -a tapeline-backend
   ```
3. Reddit poller code (not yet shipped — would be a separate PR) adds
   the PRAW polling worker.
...
- The `/app/inbox` admin UI works (no inbound is auto-logged but the
