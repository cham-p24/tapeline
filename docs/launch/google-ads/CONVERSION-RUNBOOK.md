...
**Card-required at first sign-in.** This was drafted as "card-required trial on PAID traffic only" (no-card trial→paid ≈ 8.9%; card-required ≈ 31%, ~3.5×). It shipped **universally, not paid-only**: `tier.py:CARD_GATE_START = 2026-08-22`, `tier.must_add_card()`. Every account created on/after that date meets the `/app/start` card wall before it can use the logged-in product — Stripe Checkout, $0 that day, 14-day Premium trial, first charge on day 14, one click to cancel. Accounts created before the cutover are grandfathered forever. There is **no remaining no-card path** for organic, so this is not an A/B to run. **Open item:** §2.7's LTV:CAC and payback figures were computed against the old no-card conversion rate and need recomputing against the card-gated one before the scale gate means anything.
