...
| **First paid charge** | Nothing | **No Meta event.** *Row corrected 2026-09-17: #834 (merged 2026-09-15) changed the code this row described.* The moment is now detected on `invoice.payment_succeeded`, not on subscription status: `_welcome_on_first_paid_invoice` claims the `paid_start:{sub_id}` latch for the subscription's first invoice with `amount_paid > 0`. A trial that turns `active` about an hour before its charge no longer counts; a first charge that clears through dunning does. The same event still handles dunning recovery. It sends only the welcome email and the founder's revenue alert, with no Meta or GA4 event. (Written 2026-09-13, this row said the latch fired when a subscription first became active and that `invoice.payment_succeeded` only handled dunning recovery; both stopped being true with #834.) | **Confirmed gap** (P5 below) |
...
- **Correction, 2026-09-14:** the E bullet above wrongly cleared E's record line: "We publish every day" was false (the record has trading days with no top ten and dated corrections), as were E's "never re-ranked, back-filled or removed" and "written down when it prints"; E was rewritten in PR #836 (see `docs/ads/2026-09-video/README.md`).
