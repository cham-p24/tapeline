...
Your wedge is *"the only retail scanner that publishes its methodology — six named factors and their weight ordering — and back-checks every pick vs SPY publicly, winners and losers."* (The exact weights and the scoring equation stay unpublished; PR #342 withdrew them, so never phrase the wedge as "publishes its formula".) It is genuinely defensible and powers five channels at once (SEO asset + FinTwit content + Reddit receipt + affiliate pitch + Google/E-E-A-T trust signal).
...
- The 6-factor composite is **real running code** (`score.py:compute_tapeline_composite`), weights sum to 1.0, ~2,500-ticker universe, and a refresh this line used to call "sub-60s". (Resolves the May scoring-audit complaint.) *[Corrected 15 September 2026: the universe is about 11,500 US stocks and ETFs, prices are delayed about 15 minutes, and a pass lands about every 60 seconds, which is not sub-60s either. See `docs/COPY_FACTS.md`.]*
...
**The governing reality (two web briefs reached this independently): finance CPC is brutal.** Avg finance CPC ~$3.44; head terms $4–8+; "stock screener" near the top; finance CVR ~7.5%. Math: $6 CPC × 7.5% click→trial × 25% trial→paid ≈ **~$320 CAC**. That payback figure was originally computed against the retired $29.99 tier; at live pricing ($9.99 Pro / $19.99 Premium) the same CAC is a **16–32-month gross payback** — far longer than likely retail-tool lifetime. **Do NOT lead with broad paid Search.** Lead with SEO + earned; use paid only surgically.
...
- ⚠️ **Superseded by #536/#548 (2026-08-21/22):** the trial is card-required for every new account, so the volume/email-capture assumption below has to be re-derived against a card-gated funnel — the account-free public record (`/daily-picks`, `/scorecard`, `/t/{sym}`) is now the volume surface. The rest still holds: the real risk isn't farming (the abuse stack is strong) — it's **low trial→paid conversion.** Instrument trial→paid *by cohort* (`signup_health.conversion_pct`). The data-extraction throttle during trial is smart — keep it, but note `tier.py:_TRIAL_PREMIUM_REDUCTIONS` now holds exactly one key, `api_requests_per_day` 1,000→100 (the Telegram throttle went with the channel, and the throttle only applies to the legacy no-card trial anyway — a card-required trial carries a `stripe_customer_id` and is not throttled).
- **Pricing fixes:** ⚠️ **Superseded by the 2026-07-03 founding reprice** — Pro is $9.99/mo ($8.25 annual, $99/yr) and Premium $19.99/mo ($16.58 annual, $199/yr); the $29.99/$49.99 points this section reasoned from are retired and must never be quoted as current, and annual is already the default toggle (2026-07-18). The old recommendation to widen to $24.99/$54.99 is dead — do not re-open pricing without conversion data. Keep **Lifetime $399 ≤100 seats, Stripe-enforced, never reopened.**
...
- **Expansion (Pro→Premium):** the Premium fences (Recent insider buys (SEC Form 4), unlimited email alerts, the public API) are the right ones. *(15 September 2026: congressional trades removed from this list — there is no real source for them, #820.)* Drive upgrades with blog posts that feed the *paid* feature pages (a Form 4 explainer) + a teaser of insider data to Pro users. *(15 September 2026: a congressional-trades explainer and a congress-data teaser were removed from this sentence; there is no congressional trade data, #820.)* ("Elite 13F" is not a fence — Quiver was cancelled and the surface is retired.)
...
| **Public API tier** ($99/$499/$2499) | $5–30k MRR *if real* | ~2 wks | **Massive/Polygon + Finnhub redistribution license check** (`docs/LICENSE_AUDIT.md`) | 120–365d |
...
**Messaging throughline:** lead with the published methodology (six named factors + weight ordering) + public scorecard *including losses*. *"Most gurus show you the wins. We show you the record."* Sell **process & time-savings, not profit** — compliant on every axis simultaneously.
...
**60 days — convert & measure.** Recruit 15–20 affiliate creators. Push `/best-stocks-for/*` (pos 11–12) to page 1. Ship `/glossary`. Flip Google Ads → Target CPA; pause dead ad groups. Activate drip/alert/briefing emails (real users now exist). ~~Pricing test (Pro $24.99 / Premium $54.99; default annual).~~ **Dead** — those price points are retired (2026-07-03 founding reprice) and annual has been the default toggle since 2026-07-18; see §6, and do not re-open pricing without conversion data. Add AU disclaimer block; book AU lawyer. Fix QA crawler + GEO tracker.

**90 days — read the signal.** Read trial→paid cohort CVR. ~~If <8–10% after 200+ paid-traffic trials, A/B card-required trial on paid source.~~ **Moot since #548 (2026-08-22)** — every new account is card-gated at `/app/start`, so there is no card-free arm left to test against. Backfill Finnhub caches. Gate an API waitlist behind `/developers`; verify Massive/Polygon + Finnhub ToS (`docs/LICENSE_AUDIT.md`). Concentrate on whichever 1 channel is delivering. **Target first ~$500 MRR.**
