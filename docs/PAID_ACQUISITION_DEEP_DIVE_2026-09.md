...
**The one live gap: no business address on the site.** Google's Financial products and services policy requires the destination page to carry *"The physical address for the business offering the financial product or service"* and *"All associated fees,"* and states disclosures *"can't be posted as roll-over text or made available through another link or tab."* Fees are well disclosed (`/pricing` is explicit). The address is not: a grep of `frontend/` on 2026-09-04 finds only city/country prose — `frontend/app/press/page.tsx` and the scorecard disclaimer.

**CLOSED 2026-09-07, but NOT where this paragraph originally said to put it.** The instruction here was "put it in `MarketingFooter.tsx` so it appears on every landing page." That was wrong twice over, and following it would have left the gap open:

- **`/signup` renders no `MarketingFooter`.** Its own file header calls it "the page every paid ad lands on", and every Meta ad URL in `docs/ads/meta-burst-2026-08/` points at `/signup?from=…`. Nine of the sixty-five public routes have no footer at all (signup, signin, forgot-password, reset-password, verify-email, unsubscribe, status, preview-trial-welcome, the embed iframe).
- **Where a footer did render, the disclosure would have been late or absent.** Of the two documented Google ad-group destinations (§ `CONVERSION-RUNBOOK.md:42`), `/best-stock-scanners` carries no location string at all, and `/scorecard`'s sits below `ScorecardClient`'s early-return skeleton — so it is not in the server HTML and paints only after the fetch resolves.

The line now lives in `components/GeneralInformationNotice.tsx`, which is mounted once in `app/layout.tsx` and is therefore the only surface genuinely present on every route and in the first byte of HTML. Guarded by `frontend/__tests__/businessLocationDisclosure.test.tsx`, which pins the copy, the layout mount, and agreement with the Organization JSON-LD.

**It is city-level — "Melbourne, Victoria, Australia" — not a street.** Tapeline is a sole trader working from home, so the street address is a private residence. This satisfies the *spirit* of the requirement and is what a reviewer will most often accept; it does not satisfy the literal words "physical address", and a reviewer entitled to insist could still ask for a street. **If that happens the fix is a Melbourne virtual-office or registered-office address (~A$30–60/mo), not publishing a home address** — and it should then be matched in the Google Ads account, in Stripe, and in `lib/jsonld.ts`.
...
| 7 | ~~**Physical street address into `MarketingFooter.tsx`.**~~ **DONE 2026-09-07** — city-level line shipped in `GeneralInformationNotice.tsx` (every route, incl. `/signup`, which has no footer). See §3.4. Remaining: a street address, only if a reviewer insists — use a virtual office, never the home address. | 30 min | Google FS policy requirement, checked by manual reviewers on finance accounts. |
