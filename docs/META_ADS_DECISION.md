...
>
> - **Freshness.** Prices are delayed about 15 minutes (Massive Stocks Starter). During US market hours the worker re-reads them for every covered stock and ETF about every 60 seconds, and most score inputs are daily, so a score usually changes about once a day. The scanner and the other auto-refreshing in-app pages reload about once per pass during the US session (04:00 to 20:00 ET on trading days). Variant 3's "Summary stats are public and live" should read "public and current", and "live for subscribers" should read "current for Pro and Premium".
...
| Signup → trial | 100 % by construction (auto 30-day Premium trial, no card). ⚠️ **True only as of the 2026-08-20 snapshot** — #536 (2026-08-21) stopped signup granting any trial and #548 (2026-08-22) put a card wall at `/app/start` for new accounts, so this step is now a real, measurable drop-off. | Nothing *(now: the card wall and the trial opt-in)*. | No. |
...
2. "Every evening, the same question: what changed? Tapeline keeps a live 0–100 composite on its US universe and shows what moved and why. The scorecard and ticker pages are public — reading them needs no account and no card."
   Headline: "See what the scanner read — no account needed." *(Corrected after #548, 2026-08-22: the original close — "The Free tier shows the top of the ranked list, live, no card" — was falsified two days after it was written, because a new account now meets the `/app/start` card wall before it can use the logged-in product. Re-run `node scripts/lint-copy-compliance.mjs` after any edit here; CI does not lint `docs/**`.)*
...
- **No copy that implies a card-free account** — the card gate is universal for new accounts since #548 (2026-08-22), so paid LPs and organic land on the same wall. The only remaining variant to test is $1-trial vs $0-card-required.
