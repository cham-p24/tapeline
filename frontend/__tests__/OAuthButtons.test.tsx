...
describe("OAuthButtons attribution carry", () => {
   ...
   * oauth_attr_{provider} cookie. The email form (signup/page.tsx) forwarded
   * all four. These tests pin that OAuth now forwards the same four.
   */
  it("forwards signup_referrer_host and signup_landing_path to every provider start link", async () => {
    vi.mocked(getStoredReferrerHost).mockReturnValue({
    ...
    for (const provider of ["Google", "Microsoft", "Apple"]) {
      ...
      expect(qs.get("signup_referrer_host")).toBe("copilot.microsoft.com");
      expect(qs.get("signup_landing_path")).toBe("/glossary/rsi");
    }
  });
  ...
  it("carries all four captures plus ?next= together without clobbering", async () => {
    ...
    expect(qs.get("signup_referrer_host")).toBe("chat.openai.com");
    expect(qs.get("signup_landing_path")).toBe("/compare/finviz");
  ...
  it("omits the keys entirely when nothing is stored (direct traffic)", async () => {
    ...
    const link = await screen.findByRole("link", { name: /Continue with Google/ });
    const href = link.getAttribute("href") ?? "";
    expect(href).not.toContain("signup_referrer_host");
    expect(href).not.toContain("signup_landing_path");
    expect(href.endsWith("/api/auth/oauth/google/start")).toBe(true);
  });
});
