...
export const FACTORS: Factor[] = [
  ...
  },
  {
    ...
    limitations: [
      ...
      "A company can have several listed securities under one SEC identifier — share classes, preferred shares, notes. From 19 September 2026 a filing's transactions count for every listed common-stock class of the company, so the reading is the company's and its classes share it: Alphabet's insiders file under GOOGL, and GOOG carries the same reading. Its preferred shares, notes, warrants, rights, units and exchange-traded notes have no reading rather than a borrowed one, and neither does an ETF. Which listing is common stock is judged from its name and ticker, because no data field we hold states it, so a preferred share named and lettered like its company's common stock can be counted as common. From 17 to 19 September 2026 a filing counted only for the ticker it named, so the other common class had no reading. Before 17 September every ticker of the issuer received all of them. Each ticker moves to the current rule at its next re-check — about two days for a stock, up to about a month for an ETF, ETN or futures contract.",
