...
const TOOLS: Tool[] = [
  {
    ...
    price: "Free · $8.25/mo Pro · $16.58/mo Premium (annual) · 30-day Premium trial",
    scoring: "Named factors",
    scorecard: "Per-pick public",
    tagline:
      "The only scanner that names all six scoring factors AND back-checks each recorded daily top-10 pick next-day vs SPY.",
  },
  {
    rank: 2,
    name: "Finviz Elite",
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Stockanalysis.com offers the most usable free tier — full screener access, clean fundamental tables, ETF coverage, no paywall on basics. TradingView's free tier covers charting and a basic screener well. Tapeline is a different shape and worth being plain about: its published output is free to read with no account — the daily Top 10, the full scorecard, a page per scored ticker and the raw CSV/JSON — and a free plan (an email and a password) runs the scanner, showing the top ten scored rows of any scan rather than every match. Each is honest about what's included.",
  },
  {
    q: "What's the best stock scanner with a public track record?",
    a: "Tapeline is the only tool on this list that auto-publishes each recorded daily top-10 pick with the realized next-day return vs SPY at /scorecard, open to everyone 7 days after the session. Most competitors report aggregate statistics; few publish a dated, per-pick record. If audit-able performance is the deciding factor, Tapeline is the only fit.",
  },
  {
    ...
    <main id="main" className="min-h-screen">
      ...
      <article className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        ...
        <p className="mt-4 text-lg text-muted">
          ...
          See the preview below, then read the full ranking — we're upfront about which
