...
const METHODOLOGY_LOG: LogEntry[] = [
  ...
  // are in backend/app/services/scorecard_export.py KNOWN_LIMITATIONS.
  {
    ...
    body:
      "At 13:30 UTC on 14 September 2026, 856 tickers held a Smart Money value although no SEC Form 4 filing for them was on file: 629 ETFs, 222 stocks and 5 futures funds. 42 of those values are below 10 or above 90, which the Form 4 calculation cannot produce. Where the values came from has not been established. 16 of the 100 entries recorded from 24 August to 11 September 2026 were ranked while the ticker held such a value: BBH on 24, 25, 26 and 28 August and 1, 3 and 8 September; BBP on 25, 26 and 28 August and 1 September; BIB on 26 August and 1 September; and PLX on 8, 10 and 11 September. Each of those four values is the same on every day of our stored score history, which begins on 24 August 2026, so lists recorded before that date cannot be checked. For these tickers two statements were not true: the Smart Money methodology page says a ticker with no disclosed filings in the window has no reading at all, and the 2026-08-23 entry for #620 says a missing reading stays blank. From 14 September 2026 (#824), when our data vendor reports no Form 4 filings for a ticker in the window, its Smart Money value is removed, its stored filings are deleted, and its score is recomputed with that factor counted as neutral. A reading whose filings have all left the window is removed at the ticker's next re-check, not at once. A ticker holding a value with no filing on file is re-checked at the next daily run instead of on its usual schedule, which for ETFs is about monthly (#FOLLOWUP). No recorded entry was changed.",
    ref: "#824",
  },
...
const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    body: [
      // A sweep for the 30-day change rewrote "14-day" here. That was wrong:
      // this entry is dated 22 August 2026, when the trial WAS 14 days and
      // the card was taken at first sign-in. Restored. A changelog records
      // what was true on its date; supersede it, never edit it.
      // copy-compliance-allow * -- dated 2026-08-22 entry; on that date the card was taken at first sign-in and the trial was 14 days
      ...
      // copy-compliance-allow stale-trial-length -- same dated entry
