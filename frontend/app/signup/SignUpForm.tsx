...
// This component is rendered by app/signup/page.tsx, which is a SERVER
// component marked `dynamic = "force-dynamic"`. That is load-bearing: under
// STATIC prerendering Next bails out of any subtree calling useSearchParams()
// and ships only the Suspense fallback, so /signup served a heading and NO
// form at all — zero <input> elements in 24KB of HTML, with the real form
// appearing only after ~200KB of JS executed. Paid traffic on a phone saw a
// blank page, and the Meta pixel (afterInteractive) fired only after
// hydration, which is why 27 ad clicks recorded 14 landing-page views.
// Rendering this route per-request makes useSearchParams() resolve during SSR,
// so the form is in the first byte of HTML. The Suspense boundary below stays
// as the belt-and-braces fallback if the route is ever made static again.
//
