    ...
    <main id="main" className="min-h-screen">
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <div className="prose prose-invert mt-8 max-w-none text-sm leading-relaxed text-muted">
          ...
          <p>
            We collect the minimum personal data needed to run the product: your email and password for the account; your name, watchlist, alerts and subscription state for the features that need them. We do not store IP addresses or browser fingerprints to the database, and do not see your payment-card details (Stripe handles them). For product analytics and advertising measurement we use Google Analytics 4 and Google Ads, which set cookies and receive limited usage and conversion data. If we enable Meta advertising, our servers would additionally send Meta a hashed version of your email address when you sign up, start a trial, or are charged — hashing is not anonymisation, since Meta matches those hashes against its own records. All of it is detailed in the Cookies and Sub-processors sections below. We do not sell your personal data, and we do share limited advertising-measurement data as described there.
          </p>

          <h2 className="mt-8 text-lg font-semibold text-fg">What we collect at signup</h2>
          <ul className="list-disc pl-5 space-y-1">
            <li><strong>Email address</strong> — required. Used for authentication, transactional email (welcome, trial reminders, alerts you&rsquo;ve opted into), and account recovery. If we enable Meta advertising, a one-way SHA-256 hash of it would also be sent to Meta as a conversion signal — never the address itself. See <em>Sub-processors</em>.</li>
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li><strong>Meta (Facebook &amp; Instagram)</strong> — advertising measurement. <em>Not currently enabled — no Meta code runs on this site today and nothing has ever been sent to Meta.</em> If we switch it on, Meta is not a vendor acting only on our instructions: it decides its own advertising purposes and may combine what it receives with data it already holds about you. There would be two separate flows. <strong>From our servers:</strong> when you create an account, when a trial starts, and when a subscription is charged, we send one event containing a SHA-256 hash of your email address, a hash of our internal account ID, the event name, a timestamp, a de-duplication ID, a currency, and — depending on the event — the amount charged, the plan, or how you signed up. We do <em>not</em> send your raw email address, your name, your IP address, your browser user-agent, or which page you were on. Hashing is not anonymisation: the whole point of sending a hash is that Meta matches it against its own records, so treat this as a disclosure of personal data. <strong>From your browser:</strong> Meta's script would run on our public marketing pages only — deliberately never on the signed-in app, so it cannot see which tickers you look at — set the cookies described under <em>Cookies</em>, and report each page view. Loading that script tells Meta your IP address, browser and language, and because the request goes to facebook.com your browser may attach Facebook cookies it already holds, which can let Meta link the visit to your logged-in Facebook or Instagram account. That happens between your browser and Meta; we neither see nor store it, and a tracker-blocking extension prevents it.</li>
            ...
            <li><strong>Microsoft Clarity</strong> — session replay and heatmaps. <em>Not currently enabled.</em> If we switch it on it records how you move through pages.</li>
            <li><strong>Plausible</strong> — privacy-focused traffic analytics. <em>Not currently enabled.</em> It is cookie-less and records no per-person identifier.</li>
