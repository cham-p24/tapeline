...
/** Site-wide social card, generated by app/opengraph-image.tsx. */
...
export function pageMeta(args: PageMetaArgs): Metadata {
  ...
  const openGraph: NonNullable<Metadata["openGraph"]> = {
    ...
    type: ogType,
    locale: "en_US",
    // Default to the site card when a route ships no image of its own.
    //
    // This helper builds a COMPLETE openGraph object, and a page-level
    // openGraph REPLACES the layout's rather than merging into it — which is
    // the whole reason this helper exists (see the header comment). So adding
    // `images` to the root layout was not enough: every route that calls
    // pageMeta() overwrote it with an images-less object and still emitted no
    // og:image. Verified live after the first attempt shipped — /limitations,
    // /why and /glossary had og:title and og:description but no og:image.
    images: [{ url: args.ogImage ?? DEFAULT_OG_IMAGE }],
  };
  ...
  return {
    ...
    twitter: {
      ...
      title: args.title,
      description: args.description,
      // Same reasoning as openGraph above. `card: "summary_large_image"` is a
      // promise that an image exists; without this it was a bare grey box.
      images: [args.ogImage ?? DEFAULT_OG_IMAGE],
    },
  };
}
