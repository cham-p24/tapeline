...
Adds the audit + cost-tracking layer on top of the Phase A `inbound_messages`
table (migration 0026). One row per LLM classification call. Rule-based
fast-path matches don't write here — the table is exclusively for LLM-
backed decisions where we want:
...
Revision ID kept at 29 chars to stay under the version_num VARCHAR(32) limit
(see memory: tapeline_alembic_version_limit).

Revision ID: 0028_inbox_classification_log
...
revision = "0028_inbox_classification_log"
down_revision = "0027_password_reset_bounce"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        # 'cap-exceeded' / 'no-api-key' when the LLM was deflected.
        sa.Column("model", sa.String(length=64), nullable=False),
        sa.Column("input_tokens", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("output_tokens", sa.Integer(), nullable=False, server_default="0"),
        # Cached tokens reported separately so we can verify prompt caching
        # is actually firing (warm-cache should hit ~95% of input).
