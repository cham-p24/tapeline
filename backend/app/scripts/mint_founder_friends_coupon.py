...
def main() -> int:
    ...
    logger.info("  2. They sign up at tapeline.io/signup — email and password, no card.")
    logger.info("     The free plan is live for them from that moment; nothing is walled off")
    logger.info("  3. When they want the trial they start it themselves at /app/billing")
    logger.info("     — that is where the card goes in ($0 today, first charge day 14)")
    logger.info("  4. BEFORE day 14 they go to /app/billing → Premium → enter FOUNDERFRIENDS")
    logger.info("  5. Stripe applies the 50% off for the next 3 monthly invoices")
