...
Four orthogonal toggles the inbox worker checks before any expensive
or externally-visible action:

  1. **Global enable** (`INBOX_BOT_ENABLED`) — master switch. When false,
     classification + delivery both skip; pollers still poll and log.
     Use to instantly pause everything without a redeploy.
  2. **Per-channel enable** (`INBOX_REDDIT_ENABLED` etc.) — narrower switch
     for when one channel goes haywire (Reddit account shadow-banned,
     Resend webhook spam-loop) but the others are healthy.
  3. **Dry-run** (`INBOX_DRY_RUN`) — classify + decide normally, but every
     channel adapter short-circuits before the actual send. Logs the
     "would have sent" payload. Use for shadowing the bot for a week
     before going live.
  4. **Daily Claude cost cap** (`INBOX_CLAUDE_DAILY_CAP_USD`) — once
     ...
     review and logs `model='cap-exceeded'`. Resets at UTC midnight.
     Caches the answer for 60s so we don't add a DB round-trip to every
     classify() call.
