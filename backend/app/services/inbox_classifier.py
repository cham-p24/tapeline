...
API error), we ALWAYS return Tier 1 with `suggested_reply=None`. Founder
reviews manually. We never auto-send a reply we couldn't classify.
...
def classify(body: str, *, author: str | None = None, channel: str | None = None) -> ClassifiedMessage:
    """Synchronous classifier — rule-based path only. For unit tests and
    sync callers that can't await a DB session.

    Ambiguous messages (no rule match) return Tier 1 with a safe default
...
async def classify_with_llm(
    ...
    *,
    ...
    # cached rate. We collapse both into `cached_tokens` for the spend
    # calc, but log them separately if the SDK exposes them.
    usage = getattr(response, "usage", None)
    input_tokens = int(getattr(usage, "input_tokens", 0) or 0)
    output_tokens = int(getattr(usage, "output_tokens", 0) or 0)
    cache_read = int(getattr(usage, "cache_read_input_tokens", 0) or 0)
    # Note: anthropic.types may expose cache_creation_input_tokens; if so
    # it's already inside input_tokens per the API docs, so don't double-
    # count.
