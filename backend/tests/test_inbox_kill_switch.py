"""Phase A.5 coverage for the inbox kill-switch + spend-cap helpers.
...
def _clear_spend_cache():
    """Each test starts with a clean in-process cache."""
...
def _reset_settings_cache():
    """Settings are @lru_cache'd; tests that monkey-patch env need a
    fresh instance. This clears the cache so each test sees the
    overridden value."""
...
class TestEnableSwitches:
    ...
    def test_unknown_channel_defaults_enabled(self):
        """Belt + braces — a typo in a future channel name should NOT
        silently drop messages. Default-enabled means the bug is loud
        (auto-reply happens) rather than silent."""
        assert inbox_kill_switch.channel_enabled("nonexistent_channel") is True


class TestSpendCap:
    """`cap_exceeded()` is the cost firewall. These tests use mocked
    spend-today values so we don't need DB fixtures."""
    ...
    async def test_cap_exceeded_when_spend_exactly_at_cap(self, monkeypatch):
        """At the cap is treated as exceeded — better to under-shoot than
        burn one final classification past the line."""
        monkeypatch.setenv("INBOX_CLAUDE_DAILY_CAP_USD", "5.0")
        get_settings.cache_clear()
